...
4. **邮箱**（必填，回执和面试邀约靠它进台账）+ **模型**（默认直接用你本机的 Claude Code / Codex，不用 key；没装才会问你要一个）
...
## 模型：不需要 API key，用你本机的 Claude Code 或 Codex

分数、排序、逐条判据、动作标签全是确定性 Python，**模型不产生任何数字**。

模型只干两件事，都走你本机已有的 `claude` / `codex` 命令（用你的订阅，不要 key）：

1. **联网查公司事实**：融资阶段、融资总额、Glassdoor 评分——每个字段带出处 URL，查不到写 null 不猜。
   查回来的事实再由代码按写死的阈值打分（IPO/Series D+ 记 2、Glassdoor ≥4.0 记 2……）。
2. **写给人看的文案**（可选）：「为什么」「触达路径」。没有它排名照旧。

两个都没装 → 第四问「三年后值不值钱」标未核、不进分母，其余三问照算，排名照出。
也可以给自己的 key（`JMB_MODEL_KEY`，Anthropic 或 OpenAI 兼容格式），但不是必须。
...
模型只写文案（「为什么」「触达路径」「弹药/缺口」）和查公司事实，不出任何数字。找不到本机 `claude` / `codex` 也没给 key，就跳过，**不编**。
